...
subsets. As long as we reduce $N$, we can keep $P = 1000$ and still have
smooth interaction.
...
2.  While the `phylobar` function executes quickly, rendering and visual
    interpretation both suffer when $N$ is large. When $N$ and $P$ are
    large, the initial view will show $N \times P$ rectangles in the
    stacked barplot. Rendering all rectangles as SVG elements in the
